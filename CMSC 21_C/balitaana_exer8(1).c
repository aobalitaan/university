...
void check(char *guess, char *wordle, char *hint_array, int tries, int wordle_size)
{

	for (int i = 0; i < tries; i++)																	// Iterates through the guess
	{
...
void playWordle(char *wordle)
{
	...
	while (counter < tries)																			// Game continues while there are still tries
	{
		...
		check(guess, wordle, hint_array[counter], tries, wordle_size);								// Checks the guess for possible hints
